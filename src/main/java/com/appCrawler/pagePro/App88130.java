...
public class App88130 implements PageProcessor{
	...
	public Apk process(Page page) {
		...
		if(page.getUrl().regex("http://www.88130.cn/search.html.*").match())
		{
			List<String> apps=page.getHtml().links("//div[@class='top-list2']").all();
			page.addTargetRequests(apps);
			List<String> pages=page.getHtml().xpath("//div[@class='commentBox-pages']/a/@href").all();
			page.addTargetRequests(pages);
		}
		if(page.getUrl().regex("http://www.88130.cn/app/\\d+\\.html").match())
		{
				return App88130_Detail.getApkDetail(page);
			}
			return null;
