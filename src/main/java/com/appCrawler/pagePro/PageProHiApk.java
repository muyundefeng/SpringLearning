...
public class PageProHiApk implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://apk\\.hiapk\\.com/search\\?.*").match()) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links("//div[@class='soft_list_box']/ul/li/div/div[@class='left']").all();
     ...
     // 获取信息
        if (page.getUrl().regex("http://apk\\.hiapk\\.com/appinfo/.*").match()) {
          return PageProHiApk_Detail.getApkDetail(page);
