...
public class K61_Detail {
	private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(K61_Detail.class);
	public static Apk getApkDetail(Page page){
		...
		String str=page.getHtml().xpath("//div[@class='zgameAbout fl']/div[@class='rigame fl']").toString();
		String raw=usefulInfo(str);
		String temp[]=raw.split("：");
		
		appCategory=temp[1].replace("平台", "");
		appCategory=appCategory.replace("&nbsp;", "");
		appVersion=temp[4].replace("更新时间", "");
		appUpdateDate=temp[5];
		
		
		appDownloadUrl = page.getHtml().xpath("//div[@class='downAnd mb10']/a/@href").toString();
		appSize=page.getHtml().xpath("//div[@class='downAnd mb10']/a/em/text()").toString();
		appSize=appSize.replace("(", "").replace(")", "");
