...
public class PageProApkye implements PageProcessor {
    ...
    public Apk process(Page page) {
     ...
     // 获取搜索页面
        if (page.getUrl().regex("http://www\\.apkye\\.com/|android\\-app|games").match()) {
        	
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links().regex("http://www\\.apkye\\.com/.*").all();
    ...
    // 获取信息
    if (page.getUrl().regex("http://www\\.apkye\\.com/\\d{3,}.html").match()) {
        Apk apk = PageProApkye_Detail.getApkDetail(page);
