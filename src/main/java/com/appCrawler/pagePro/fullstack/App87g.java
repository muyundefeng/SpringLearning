...
public class App87g implements PageProcessor{
	...
	public Apk process(Page page) {
		...
		if("http://www.87g.com/".equals(page.getUrl().toString()))
		{
			page.addTargetRequest("http://www.87g.com/youxi/g-33-1-0-0-1-1.html");
			return null;
		}
	
		if(page.getHtml().links().regex("http://www.87g.com/youxi/g-33-1-0-0-1-\\d+\\.html").match())
		{
			//List<String> categoryList=page.getHtml().xpath("//div[@class='apply-menu']/ul/li/a/@href").all();
	 		List<String> apps=page.getHtml().links("//ul[@class='hf-list5 MENU']").all();
		...
		if(page.getUrl().regex("http://www\\.87g\\.com/youxi/\\d+\\.html").match())
		{
			...
			Apk apk = App87g_Detail.getApkDetail(page);
