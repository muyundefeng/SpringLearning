...
public class PageProMumayi implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://www\\.mumayi\\.com/.*").match()) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links().regex("http://www\\.mumayi\\.com/android.*").all();
        ...
        if (page.getUrl().regex("http://www\\.mumayi\\.com/android.*").match()) {
			...
			Apk apk = PageProMumayi_Detail.getApkDetail(page);
