...
public class Mgamebaidu implements PageProcessor{
	...
	public Apk process(Page page) {
		...
		List<String> urlList=page.getHtml().xpath("//div[@class='left']/a/@href").all();
		List<String> nextPage = page.getHtml().xpath("//ul[@class='dk_game_fenye_ul']//a/@href").all();
		...
		//提取页面信息
	    if(nextPage.size() <1){		
			Apk apk = Mgamebaidu_Detail.getApkDetail(page);		
