...
public class PagePro7613 implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://www\\.7613\\.com/soft/14/").match()) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links().regex("http://www\\.7613\\.com/soft/.*").all();
        ...
        if (page.getUrl().regex("http://www\\.7613\\.com/soft/.*").match()) {
           Html html = page.getHtml();
			Apk apk = PagePro7613_Detail.getApkDetail(page);
