...
public class Meizumi implements PageProcessor{
	...
	private Logger LOGGER = LoggerFactory.getLogger(Meizumi.class);

	public Apk process(Page page) {
	
		//System.out.println(page.getHtml().toString());
		List<String> urls =page.getHtml().links().regex("http://www\\.meizumi\\.com/.*").all() ;
		
 		
		Set<String> cacheSet = Sets.newHashSet();
		cacheSet.addAll(urls);
		for (String temp : cacheSet) {
			if(!temp.contains("http://www.meizumi.com/download/")
		...
		if(page.getUrl().regex("http://www\\.meizumi\\.com/apk/.*").match()){
			...
			Apk apk = Meizumi_Detail.getApkDetail(page);
