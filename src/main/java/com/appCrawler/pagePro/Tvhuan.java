...
public class Tvhuan implements PageProcessor{
	...
	public Apk process(Page page) {
		...
		if(page.getUrl().regex("http://app.tvhuan.com/search.*").match())
		{
			
			List<String> apps=page.getHtml().links("//div[@class='selfadapt']").all();
			page.addTargetRequests(apps);
		}
		if(page.getUrl().regex("http://app\\.tvhuan\\.com/detail-.*").match())
		{
				return Tvhuan_Detail.getApkDetail(page);
			}
			return null;
