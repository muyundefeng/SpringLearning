...
public class PageProLenovomm implements PageProcessor {
    ...
    public Apk process(Page page) {
        ...
        // 获取搜索页面
        if (page.getUrl().regex("http://www.lenovomm.com/search/index.html\\?q=*").match()) {
            LOGGER.debug("match success, url:{}", page.getUrl());

            // 获取详细链接，以及分页链接
            List<String> urlList = page.getHtml().links("//ul[@class='appList']/li").all();
            urlList.addAll(page.getHtml().links("//div[@class='appPageBox fl w-100']/div[@class='pagenav tcenter']").all());
        ...
        if (page.getUrl().regex("http://www\\.lenovomm\\.com/app.*").match()) {
        	return PageProLenovomm_Detail.getApkDetail(page);
